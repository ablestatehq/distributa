...
function replaceNullWithEmptyString(obj) {
  ...
  return Object.keys(obj).reduce((acc, key) => {
    acc[key] = replaceNullWithEmptyString(obj[key]);
    return acc;
  }, {});
}
...
const EditInvoice = () => {
  ...
  const { isSubmitting } = useNavigationLoadingState();
  ...
  return (
    <ContentViewAreaWrapper>
      ...
      <main className="flex w-full flex-col h-fit gap-y-4 lg:pt-6">
        <Suspense
          fallback={
            <main className="flex w-full flex-col h-fit gap-y-4 lg:pt-6 animate-pulse">
              {/* Header skeleton */}
              <div className="h-8 bg-grey rounded w-1/4 mb-4"></div>

              {/* Form skeleton */}
              <div className="bg-white p-6 rounded-lg shadow-sm">
                {/* Invoice details section */}
                <div className="mb-6">
                  <div className="h-4 bg-grey rounded w-1/4 mb-4"></div>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div className="h-10 bg-grey rounded"></div>
                    <div className="h-10 bg-grey rounded"></div>
                  </div>
                </div>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="h-6 bg-grey rounded w-1/3"></div>
                    <div className="h-8 bg-grey rounded w-1/2"></div>
                  </div>
                </div>

                {/* Action buttons skeleton */}
                <div className="flex justify-end space-x-4">
                  <div className="h-10 bg-grey rounded w-24"></div>
                  <div className="h-10 bg-grey rounded w-24"></div>
                </div>
              </div>
            </main>
        ...
        >
          <Await resolve={data?.invoice}>
            {(initialValues) => {
              initialValues = replaceNullWithEmptyString(initialValues);
              initialValues.issue_date = format(
                parseISO(initialValues.issue_date),
                "yyyy-MM-dd"
              );
              initialValues.due_date = format(
                parseISO(initialValues.due_date),
                "yyyy-MM-dd"
              );
